...
class BookingRepository extends BaseRepository
{
    ...
    /**
     * Sends push notifications to suitable translators for a job
     * @param Job $job
     * @param array $data
    ...
    public function sendNotificationTranslator(Job $job, array $data = [], int $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                ...
                foreach ($jobs as $oneJob) {
                    // check if the job is suitable for the translator
                    if ($this->isJobSuitableForTranslator($oneJob, $data)) {
                        if ($this->isNeedToDelayPush($oneUser->id)) {
                            $delpay_translator_array[] = $oneUser->id;
                        } else {
                            $translator_array[] = $oneUser->id;
                        }
                    }
                }
            }
        ...
        $this->sendPushNotificationToSpecificUsers($translator_array, $job->id, $data, $msg_text, false);       // send new booking push to suitable translators (not delayed)
        $this->sendPushNotificationToSpecificUsers($delpay_translator_array, $job->id, $data, $msg_text, true); // send new booking push to suitable translators (need to be delayed)
    }
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        if ($job->from_language_id != $data['from_language_id']) {
            $log_data[] = [
                'field' => 'from_language_id',
                'old_value' => $job->from_language_id,
                'new_value' => $data['from_language_id']
            ];
        ...
        if ($job->due <= Carbon::now()) {
            ...
            if ($langChanged)
                $this->sendChangedLanguageNotification($job, $old_lang);
            return ['Updated'];
        }
    }
    ...
    private function changeAssignedStatus($job, $data)
    {
        if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            ...
            if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24'])) {
                ...
                $job->withdraw_by = 'admin';
                $job->withdraw_comment = $data['admin_comments'];
                $job->save();
                $user = $job->user()->first();
                $email = !empty($job->user_email) ? $job->user_email : $user->email;
                $name = $user->name;
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            if (!is_null($current_translator) && ((isset($data['translator']) && $current_translator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                ...
                $new_translator = Translator::find($data['translator']);
                if (is_null($new_translator)) {
                    $new_translator = new Translator();
                    $new_translator->email = $data['translator_email'];
                    $new_translator->save();
                }

                $job->translatorJobRel()->create([
                    'translator_id' => $new_translator->id,
                    'created_at' => Carbon::now()
                ]);

                $translatorChanged = true;
                $log_data['old_translator_id'] = $current_translator->user_id;
                $log_data['new_translator_id'] = $new_translator->id;
            ...
            if ($translatorChanged) {
                // Send notification to the customer and new translator
    ...
    public function acceptJob($data, $user)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                $data = [
                    // Add any necessary data for the email template
                ];
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            return $this->getAllJobsForAdmin($request, $limit);
        } else {
            return $this->getAllJobsForAdmin($request, $limit);
        }
    }

    function getAllJobsForAdmin(Request $request, $limit = nul)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            if (isset($requestdata['customer_email']) && count($requestdata['customer_email']) && $requestdata['customer_email'] != '') {
                $allJobs->whereHas('user', function ($q) use ($requestdata) {
                    $q->whereIn('email', $requestdata['customer_email']);
                });
            }
            if (isset($requestdata['translator_email']) && count($requestdata['translator_email'])) {
                $allJobs->whereHas('translatorJobRel.user', function ($q) use ($requestdata) {
                    $q->whereIn('email', $requestdata['translator_email']);
                });
            }
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if (isset($requestdata['filter_timetype']) && $requestdata['filter_timetype'] == "created") {
                ...
                if (isset($requestdata['to']) && $requestdata['to'] != "") {
                    $allJobs->where('jobs.created_at', '<=', $requestdata['to']);
                }
            }
            if (isset($requestdata['filter_timetype']) && $requestdata['filter_timetype'] == "due") {
                if (isset($requestdata['from']) && $requestdata['from'] != "") {
                ...
                if (isset($requestdata['to']) && $requestdata['to'] != "") {
                    $allJobs->where('jobs.due', '<=', $requestdata['to']);
                }
            }
            if (isset($requestdata['job_type']) && $requestdata['job_type'] != '') {
                $allJobs->whereIn('jobs.job_type', $requestdata['job_type'])
